...
event_details = {
    ...
    "LAGUNA BEACH": {
        ...
        "доп. информация": """🏕 Размещение на территории фестиваля «LAGUNA BEACH»

Размещение на территории фестиваля осуществляется строго после покупки билета на фестиваль.

Также вы можете выбрать свободное посещение проекта или расположиться за пределами территории фестиваля.

🏕 Сейчас доступны:
• бронь места под палатку (место выбирается на месте);
• бронь шатров на территории.

⚠️ Внимание!
Бронирование шатров производится только напрямую через площадку «Территория Я».

☎ Телефон:
+7 (911) 863-08-80

📍 Адрес:
г. Балтийск, ул. 10 км от Павлово, д. 1.

🎟 В стоимость билета на фестиваль входит:

• круглосуточный проход на территорию при наличии браслета;
• возможность посещать все площадки фестиваля;
• пользование инфраструктурой;
• свободный выход к морю.

❗ Место под палатку приобретается дополнительно.

Сейчас продаются билеты на два дня фестиваля.
В ближайшие дни появится лимитированная партия билетов на один день.

⚡ На площадке будут работать:

• станции для зарядки мобильных телефонов;
• душ и тёплая вода;
• бары;
• горячая разнообразная еда;
• активности;
• релакс-зоны;
• мастер-классы.
"""}
